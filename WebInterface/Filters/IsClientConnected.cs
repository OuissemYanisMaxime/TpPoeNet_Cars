...
{
    public class IsClientConnected : ActionFilterAttribute
    {
        ...
        public override void OnActionExecuting(ActionExecutingContext filterContext)
        {
                ...
                && (filterContext.HttpContext.Request.Cookies["UserSettings"]["token"] != null))
            {
                ...
                Task<bool> t = Task.Run<bool>(async() => await "http://localhost:50631/api/IsCustomer"
                    .WithOAuthBearerToken(userToken)
                    .WithHeader("Accept", "application/json")
                    .PostAsync(null).ReceiveJson<bool>());

                if (t.Result != true)
                    filterContext.HttpContext.Response.RedirectToRoute("Default", new { controller = "Client", action = "login" });
            }
            else
            {
